...
async function getStaticPaths() {
    const client = await external_mongodb_.MongoClient.connect("mongodb+srv://suri:suri@myfirstcluster.qycng.mongodb.net/meetups?retryWrites=true&w=majority");
    const db = client.db();
    const meetupsCollection = db.collection("meetups");
    const meetups = await meetupsCollection.find({}, {
        _id: 1
    }).toArray() //find all object and return only the id fields
    ;
    client.close();
    return {
        fallback: false,
